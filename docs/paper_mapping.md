...
| §3.1.4 参考轨迹生成（位姿/扭转/加速度一致） | `get_state` / `get_pose` | `test_rest_to_rest_boundaries_and_limits` |
...
**遗留**：按步姿态参考跟踪（时变 `r_des`）待控制接口扩展；当前姿态恒定场景下 SE(3) 机制退化但数学完整。
...
3. **按步姿态参考跟踪**——SE(3)-TOPP 的姿态路径已生成，控制接口（时变 r_des）未接入。
